...
export async function GET(){const u=await getCurrentUser();if(!u)return Response.json({error:"Chưa đăng nhập"},{status:401});const r=await query<{source:string;title:string;due_date:string;owner:string}>(`
  SELECT 'Đơn hàng' source,p.name title,p.deadline::text due_date,COALESCE(x.full_name,'') owner FROM projects p LEFT JOIN users x ON x.id=p.owner_id WHERE p.deadline BETWEEN CURRENT_DATE-30 AND CURRENT_DATE+7 AND p.status NOT IN ('Hoàn thành','Đã chốt đơn','Đã giao hàng','Hủy')
