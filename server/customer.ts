...
export async function saveCustomer(client: PoolClient, data: Record<string, unknown>, userId: number, ownerId: number, incrementContact: boolean) {
  ...
  if (found.rows[0]) {
    ...
    const result = await client.query<{ id: number; contact_count: number }>(`UPDATE customers SET name=$1,phone=$2,customer_type=$3,sales_channel=$4,assigned_to=$5,last_contact_at=CASE WHEN $6 THEN NOW() ELSE last_contact_at END,contact_count=contact_count+CASE WHEN $6 THEN 1 ELSE 0 END,updated_at=NOW() WHERE id=$7 RETURNING id,contact_count`, [name, phone, data.customerType || "Khách lẻ", data.salesChannel || "Trực tiếp", ownerId, incrementContact, current.id]);
  ...
  const created = await client.query<{ id: number; contact_count: number }>(`INSERT INTO customers(normalized_name,normalized_phone,name,phone,customer_type,sales_channel,assigned_to,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id,contact_count`, [normalizedName, normalizedPhone, name, phone, data.customerType || "Khách lẻ", data.salesChannel || "Trực tiếp", ownerId, userId]);
